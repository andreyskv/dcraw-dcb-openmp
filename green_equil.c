...
//~ #define CLASS
/*#define uunsigned int Uunsigned int
 typedef unsigned char uchar;
 typedef unsigned unsigned int uunsigned int;*/
...
void CLASS green_equilibrate(float thresh)
{
  ...
  //uunsigned int height=m_Height, width=m_Width; //for RT only
...
//#pragma omp parallel for shared(image,height,width) private(top,left) schedule(dynamic)
  ...
  for (top=0; top < height-border; top += TS-border2)
    for (left=0; left < width-border; left += TS-border2) {
      unsigned int bottom = MIN( top+TS,height);
      unsigned int right  = MIN(left+TS, width);
      unsigned int numrows = bottom - top;
      unsigned int numcols = right - left;

      unsigned int row, col;
      unsigned int rr, cc, c, indx;
      int vote1, vote2;
